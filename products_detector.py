...
GRAPH_PATH = "./yolov2-tiny-obj.graph"
DETECTION_THRESHOLD = 0.40
IOU_THRESHOLD = 0.30

label_name = {0: "apple_green", 1: "apple_red", 2: "banana", 3: "coca_cola", 4: "egg", 5: "apple_green",
              6: "pepper_red", 7: "apple_red", 8: "sweets_mms",
              9: "sweets_nesquik", 10: "sweets_snickers", 11: "water"}
...
def post_processing(output, original_img, communication):
    num_classes = 12
    num_grids = 13
    num_anchor_boxes = 5
    ...
    original_results = np.reshape(original_results, (13, 13, 85))
    ...
    reordered_results = np.zeros((13 * 13, 5, 17))

    index = 0
    for row in range(num_grids):
        for col in range(num_grids):
            for b_box_voltron in range(85):
                b_box = row * num_grids + col
                b_box_num = int(b_box_voltron / 17)
                b_box_info = b_box_voltron % 17
    ...
    for box in results:
        # send message
        data = {}
        data['product'] = label_name[box[4]]
        communication.publish('products', json.dumps(data))
...
def main():
    # 1. Initialize Communication with AWS and subscribe "advice" topic
    ...
    # 3. Initialize Intel Movidius Neural Compute Stick
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        post_processing(output, original_img, communication)
